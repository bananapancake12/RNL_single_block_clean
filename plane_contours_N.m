...
xp=linspace(0,2*pi/alpha,xx);
zp=linspace(0,2*pi/beta,zz);

figure(2)
clf
plot(xp,usurface(1:xx,zcut),'b-')
hold on
plot(xp,ughost(1:xx,zcut),'r-')
plot(xp,uplus1(1:xx,zcut),'c-')
plot(xp,uplus2(1:xx,zcut),'m-')
title('x cut')
xlim([0 100/197.5])
figure(3)
clf
plot(zp,usurface(xcut,:),'b-')
hold on
plot(zp,ughost(xcut,:),'r-')
plot(zp,uplus1(xcut,:),'c-')
plot(zp,uplus2(xcut,:),'m-')
title('z cut')
xlim([0 100/197.5])

bslip=mean(mean(usurface))/((mean(mean(uplus1))-mean(mean(ughost)))/(yj1(2)-yj1(1)))

figure(1)
for i=1:npx
    for k=1:npz
        boxstartx=(i-1)*2*pi/beta/npx-1*2*pi/beta/xx/2;
        boxstartz=(k-1)*2*pi/alpha/npz-1*2*pi/alpha/zz/2;
        %rectangle('Position',[boxstartx boxstartz 2*pi/alpha/npz/gfz 2*pi/beta/npx/gfx],'FaceColor',[.9 .9 .9])
        rectangle('Position',[boxstartx boxstartz 2*pi/alpha/npz/gfz 2*pi/beta/npx/gfx],'EdgeColor','w')
    end
end
